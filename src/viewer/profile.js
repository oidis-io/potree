...
// TODO(mkelnar) full refactor of profile window/control structure (optimally remove jquery)

export class ProfileControl extends EventDispatcher {
    constructor(viewer, renderArea) {
        super();

        this.viewer = viewer;
        // TODO(mkelnar) make it same type as jqueried one in window till refactoring #2
                ...
                if (closest) {
                    ...
                    for (let attributeName of Object.keys(point)) {
                        ...
                        if (attributeName === "position") {
                            let values = [...position];
                            attributes[attributeName] = { x: values[0], y: values[1], z: values[2] };
...
export class ProfileWindow extends ProfileControl {
    ...
    initControl() {
        ...
        let cwIcon = `${PotreeConfig.resourcePath}/icons/arrow_cw.svg`;
        $("#potree_profile_rotate_cw").attr("src", cwIcon);

        let ccwIcon = `${PotreeConfig.resourcePath}/icons/arrow_ccw.svg`;
        $("#potree_profile_rotate_ccw").attr("src", ccwIcon);

        let forwardIcon = `${PotreeConfig.resourcePath}/icons/arrow_up.svg`;
        $("#potree_profile_move_forward").attr("src", forwardIcon);

        let backwardIcon = `${PotreeConfig.resourcePath}/icons/arrow_down.svg`;
        $("#potree_profile_move_backward").attr("src", backwardIcon);

        let dxf2DIcon = `${PotreeConfig.resourcePath}/icons/file_dxf_2d.svg`;
        $("#potree_download_dxf2D_icon").attr("src", dxf2DIcon);

        let dxf3DIcon = `${PotreeConfig.resourcePath}/icons/file_dxf_3d.svg`;
        $("#potree_download_dxf3D_icon").attr("src", dxf3DIcon);

        let csvIcon = `${PotreeConfig.resourcePath}/icons/file_csv_2d.svg`;
        $("#potree_download_csv_icon").attr("src", csvIcon);

        let lasIcon = `${PotreeConfig.resourcePath}/icons/file_las_3d.svg`;
...
export class ProfileControlController {
    constructor(viewer) {
        this.viewer = viewer;
        this.profileView = viewer.profileControl;
        this.profile = null;
        this.numPoints = 0;
        ...
        const rotate = (radians) => {
            ...
            this.profileView.autoFitEnabled = false;
    ...
    progressHandler(pointcloud, progress) {
        for (let segment of progress.segments) {
            this.profileView.addPoints(pointcloud, segment.points);
...
export class ProfileWindowController extends ProfileControlController {
    constructor(viewer) {
        super(viewer);

        this.profileView = viewer.profileWindow;
